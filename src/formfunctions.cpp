...
int formfunctions(SNES snes, Vec X, Vec F, void* ctx)
{
    ...
    vector3D gradns[7], gradNq[7], gradTe, gradne, gradTs, gradNn, gradTn;
        ...
        for (j = ys; j< ys+ym; j++) {
            ...
            for (i = xs; i < xs+xm; i++) {
                ...
                for (s = 0; s < sl; s++) {
                    ...
                    gradNq[s] = gradient(xn, i, j, k, yj, xi, s+20);
                    ...
                    ff[k][j][i].fx[s+20]= xx[k][j][i].fx[s+20] - xn[k][j][i].fx[s+20]
                                         +dt*( div_un + unr_n*gradNq[s].r + unth_n*gradNq[s].t + unph_n*gradNq[s].p
